...
So first let's start out editing the `hardware-configuration.nix` file to amend some filesystem
options:
...
lets leak through some information ... an attacker might e.g. guess the filesystem type from the
pattern of unused blocks.  For me this doesn't matter much but YMMV ;-)

So let's continue and edit (and add) `configuration.nix`.  First some general system configuration:
...
later) you'd like to have installed:
...
* I went with `xfsettingsd`, the settings daemon from Xfce
